...
| `client_id` | `string` | Payer address or identity key extracted from the payment header. Never log raw API keys or full payment headers — use truncated or hashed identifiers. |
...
:::caution
Never log raw API keys, full payment authorization headers, or other secrets. The `client_id` field should contain a truncated payer address (e.g. `0x1234...abcd`), not a full credential. If you ship logs to a third-party service, review which fields are indexed and who has access.
:::
